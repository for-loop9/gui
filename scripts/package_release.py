...
def main():
	...
	if os.name == 'nt':
		os.rename("build/bin/app.exe", f"build/bin/{project_name}.exe")
		executable_name = f'{project_name}.exe'
	else:
		os.rename("build/bin/app", f"build/bin/{project_name}")
	...
	if os.name == 'nt':
		archive_name = f'{project_name}-{args.version}-win-x86_64'
	else:
		archive_name = f'{project_name}-{args.version}-linux-x86_64'
