...
bool ig_context_begin(ig_context* context, ig_window* window, float resolution, int vsync) {
	...
	VkResult acquire_result = vkAcquireNextImageKHR(context->device, context->swapchain, UINT64_MAX, current_frame->image_available, VK_NULL_HANDLE, &context->img_idx);

	if (acquire_result == VK_ERROR_OUT_OF_DATE_KHR) {
		ig_context_resize(context, window, resolution, vsync);
		return false;
...
void ig_context_end(ig_context* context, ig_window* window, float resolution, int vsync) {
	_ig_frame* current_frame = context->frames + context->frame_idx;
	...
	VkResult present_result = vkQueuePresentKHR(context->queue, &(VkPresentInfoKHR) {
...
void ig_context_resize(ig_context* context, ig_window* window, float resolution, int vsync) {
	...
	context->default_frame.resolution.x = context->surface_extent.width * resolution;
	context->default_frame.resolution.y = context->surface_extent.height * resolution;
